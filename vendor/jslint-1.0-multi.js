...
(function (args) {
  ...
  var j, i, e, error, file, input = null, errorFiles = [];
    print("JSlint starting");
    for (j = 0; j < args.length; j += 1) {
        file = args[j];
        input = readFile(file);
        if (!input) {
            print("\033[31mCouldn't open file '" + file + "'.\033[0m");
            quit(1);
        }
        if (!JSLINT(input, {
            browser: true,
            ...
            if (JSLINT.errors.length > 0) {
                java.lang.System.out.print("\033[31m" + "F" + "\033[0m");
                errorFiles.push({
    ...
    if(errorFiles.length > 0) {
        print("\033[31m" + "Failures:\n" + "\033[0m");
    }

    for (i = 0; i < errorFiles.length; i += 1) {
        error = errorFiles[i];

        print('\033[31mErrors for file ' + error.file + ':\033[0m');

        for (j = 0; j < error.errors.length; j += 1) {
            print("" + (j + 1) + ")");
            e = error.errors[j];
            if (e) {
                print('\033[31mLint at line ' + (e.line + 1) + ' character ' + (e.character + 1) + ': ' + e.reason);
    ...
    print("" + args.length + " files, " + errorFiles.length + " failures");
